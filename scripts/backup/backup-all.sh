...
docker exec peruHCE-db-master mysqldump \
  --user=root \
...
docker exec peruHCE-db-replic mysqldump \
  --user=root \
  --password=${MYSQL_ROOT_PASSWORD:-openmrs} \
  --all-databases \
  --single-transaction \
...
docker exec peruHCE-keycloak-db pg_dump \
...
docker run --rm \
  -v peruhce-distro-referenceapplication_orthanc-data:/data \
...
docker run --rm \
  -v peruhce-distro-referenceapplication_prometheus-data:/data \
  -v "$CURRENT_BACKUP":/backup \
...
docker run --rm \
  -v peruhce-distro-referenceapplication_grafana-data:/data \
  -v "$CURRENT_BACKUP":/backup \
...
# rsync -avz "$CURRENT_BACKUP" user@backup-server:/backups/peruhce/
